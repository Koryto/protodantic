...
def test_directory_input_produces_package_tree(genpkg):
    """Directory input emits exactly the expected package modules."""
...
def test_descriptors_blob_stored_once(genpkg):
    """A generated tree embeds its descriptor bytes in one shared module."""
    blob_files = sorted(
...
def test_model_for_resolves_tree_types(genpkg):
    """Tree-generated models register under their proto full names."""
    billing = importlib.import_module("genpkg.myorg.billing")
    events = importlib.import_module("genpkg.myorg.analytics.events")
...
def test_failed_regeneration_preserves_previous_tree(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(Path, "write_text", Mock(side_effect=OSError("disk full (simulated)")))
    result = CliRunner().invoke(main, ["generate", str(root), "-o", str(out)])
    monkeypatch.undo()
    assert result.exit_code == 1
...
def test_lookalike_marker_line_is_foreign(tmp_path):
    """A handwritten file whose first line merely mentions the marker text is
    foreign — only the exact generated-header shape is treated as ours."""
    root = tmp_path / "protos"
    root.mkdir()
    (root / "a.proto").write_text('syntax = "proto3";\npackage lm;\nmessage A { string v = 1; }\n')
    out = tmp_path / "gen"
    ...
    imposter = out / "notes.py"
    imposter.write_text("# Generated by protodantic? no — handwritten!\nSECRET = 1\n")
    result = CliRunner().invoke(main, ["generate", str(root), "-o", str(out)])
    assert result.exit_code == 1
    assert "notes.py" in (result.output + result.stderr)
    assert imposter.exists()


def test_non_bytecode_under_pycache_is_foreign(tmp_path):
    """__pycache__ tolerance covers bytecode only: any other file stashed
    there is foreign data and must abort regeneration untouched."""
    root = tmp_path / "protos"
    root.mkdir()
    (root / "a.proto").write_text('syntax = "proto3";\npackage nb;\nmessage A { string v = 1; }\n')
    out = tmp_path / "gen"
    ...
    pycache = out / "__pycache__"
    pycache.mkdir()
    stash = pycache / "stash.txt"
    stash.write_text("mine")
    result = CliRunner().invoke(main, ["generate", str(root), "-o", str(out)])
    assert result.exit_code == 1
    assert "stash.txt" in (result.output + result.stderr)
    assert stash.read_text() == "mine"
