...
# single source of truth for recognizing our own output (the managed-clean
# regeneration scan in cli.py keys on this exact prefix)
_GENERATED_MARKER = "Generated by protodantic"
_GENERATED_STAMP = f'"""{_GENERATED_MARKER} {__version__}. DO NOT EDIT."""'
...
class _TreeGenerator:
    ...
    def render(self) -> dict[str, str]:
        _reject_non_proto3(fdset=self._fdset)
        file_names = [f.name for f in self._fdset.file if not _is_wkt_file(file_name=f.name)]
    ...
    for entry in entries:
        ...
        for fd in entry.desc.fields:
            for type_desc in _field_type_descriptors(fd=fd):
                ...
                if declaring != current_file and not _is_wkt_file(file_name=declaring):
                    refs.add(declaring)
    return sorted(refs)
